...
Zed is available on [Flathub](https://flathub.org/apps/dev.zed.Zed) as `dev.zed.Zed`. Like the other packages above it is maintained by community members rather than by Zed, so it may lag behind releases. It runs inside the Flatpak sandbox by default. This means Zed only sees the tools and files that the sandbox exposes, so language servers, formatters and other programs installed on your host may not be available.

The first time you start Zed inside the sandbox it offers to restart on the host, which gives it access to your normal development environment. The same choices are available later from the command palette:

- `flatpak: restart on host` leaves the sandbox for this launch only.
- `flatpak: always run on host` starts outside the sandbox from now on.
- `flatpak: always run in sandbox` reverts that, and puts Zed back in the sandbox.

To see where the running instance is, open {#action zed::About}; it reports either `Flatpak sandbox` or `Flatpak, running on host`.

The "always" choices are stored as a Flatpak environment override, so you can also set one without Zed:
